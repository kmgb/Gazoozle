...
class BulletImpactListener : public IGameEventListener2
{
	...
	{
		auto userIdx = g_EngineClient->GetPlayerForUserID(event->GetInt("userid"));
		if (userIdx == g_EngineClient->GetLocalPlayer())
		{
			Vector hit;
			hit.x = event->GetFloat("x");
			hit.y = event->GetFloat("y");
			hit.z = event->GetFloat("z");

			g_DebugOverlay->AddLineOverlay(g_LocalPlayer->GetEyePos(), hit, 0, 255, 0, true, 1.f);
			// Causes vertex buffer overflow :(
			//g_DebugOverlay->AddBoxOverlay(hit, Vector(-2, -2, -2), Vector(2, 2, 2), QAngle(0, 0, 0), 0, 255, 0, 255, 1.f);
		}
	}
...
namespace Hooks
{
	...
	BulletImpactListener bullet_listener;
	...
	void Initialize()
	{
		...
		clientmode_hook.hook_index(index::ClientMode_CreateMove, hkClientMode_CreateMove);

		g_GameEvents->AddListener(&bullet_listener, "bullet_impact", false);
	...
	/*void __stdcall hkCreateMove(int sequence_number, float input_sample_frametime, bool active, bool& bSendPacket)
		...
		//--------------------------------------------------------------------------------
	bool __fastcall hkClientMode_CreateMove(IClientMode *clientmode, int edx, float inputSampleTime, CUserCmd *cmd)
	{
		bool *pbSendPacket {};
		void *pInput {};

		__asm mov pInput, ebp;
		pbSendPacket = *(bool **)pInput - 0x1C;
		bool &bSendPacket = *pbSendPacket;

		auto oCreateMove = clientmode_hook.get_original<ClientMode_CreateMove>(index::ClientMode_CreateMove);

		bool ret = oCreateMove(clientmode, inputSampleTime, cmd);

		if (cmd->command_number == 0 || !g_EngineClient->IsInGame())
			return ret;
		...
		if (g_Options.misc_fastwalk)
		{
			if (g_LocalPlayer->m_fFlags() & FL_ONGROUND && cmd->buttons & IN_SPEED)
			{
				cmd->buttons &= ~IN_SPEED; // Remove the speed restriction

				Vector movement = {cmd->forwardmove, cmd->sidemove, 0.f};

				if (movement.Length2D() != 0.f)
				{
					...
					{
						//g_LocalPlayer->m_vecVelocity().Dot(movement);
						if (float v = (g_LocalPlayer->m_vecVelocity() + movement * g_GlobalVars->frametime).Length2D(); v > 133.734f)
						{
							// We're about to go over the step speed, slow a little
							cmd->buttons |= IN_SPEED;
							movement.x = movement.y = 0.f;
						}
					}

					if (movement.x > 450.f || movement.y > 450.f)
					{
						movement.NormalizeInPlace();
						movement *= 450.f;
					}
			...
			if (g_EngineClient->IsInGame() && !g_EngineClient->IsTakingScreenshot())
			{
				...
				if (g_Options.esp_enabled)
				{
					for (auto i = 1; i <= g_EntityList->GetHighestEntityIndex(); ++i)
					{
						...
						if (i != 0 && i < g_EngineClient->GetMaxClients() + 1 && !entity->IsDormant() && entity->IsAlive())
						{
				...
				if (g_Options.esp_inaccuracy_circle)
				{
					if (auto weapon = g_LocalPlayer->m_hActiveWeapon().Get(); weapon)
					{
						auto inacc = weapon->GetInaccuracy() * 550.f;

						int w, h;
						g_EngineClient->GetScreenSize(w, h);
						g_VGuiSurface->DrawSetColor(0, 0, 0, 120);
						g_VGuiSurface->DrawOutlinedCircle(w / 2, h / 2, inacc, g_Options.esp_circle_segments);
					}
				}
